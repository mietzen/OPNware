...
- **editor** — the shared package owning the vendored Monaco + TextMate editor tree served at `/usr/local/opnsense/www/js/vendor`; os-caddy and os-homer depend on it; its version tracks the vendored monaco-editor release; refreshed manually via `scripts/refresh-editor.sh`.
