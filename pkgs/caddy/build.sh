...
# The FreeBSD port sets CustomVersion so `caddy version` reports the
# pinned release (modules.php parses the first token to pin rebuilds).
GOOS=freebsd GOARCH="${ARCH}" CGO_ENABLED=0 go build \
    -ldflags="-X github.com/caddyserver/caddy/v2.CustomVersion=v${VERSION}" \
    -o "${DIST_ROOT}/dist/pkg/usr/local/bin/caddy" ./cmd/caddy
echo "::endgroup::"
...
# Binary
chmod 0755 "${DIST_ROOT}/dist/pkg/usr/local/bin/caddy"
