 ...
 *
 * Results are written to /var/db/os-caddy-advanced/modules_result.json and echoed as
...
function declared_modules()
{
    ...
    $modules = array();
    if (isset($config->OPNsense->caddyadvanced->general->Modules)) {
        $modules = explode("\n", (string)$config->OPNsense->caddyadvanced->general->Modules);
    }
    $seen = array();
    $result = array();
...
function rebuild($modules)
{
    ...
    // The rebuild requires go126 (some modules need 1.26); its binary lives
    // at /usr/local/go126/bin/go, off the default PATH, so a plugin-owned
    // `go` shim is put first. There is no fallback toolchain anymore: the
    // go metapackage and go125 are retired (ticket #221).
    $goBinDir = STATE_DIR . '/gobin';
    $go = '/usr/local/go126/bin/go';
    if (!is_file($go)) {
        flock($lock, LOCK_UN);
        fclose($lock);
        fail('go126 toolchain not found at ' . $go . ' (install opnware/go126)');
    }
    ...
    // Ship the full xcaddy/go output so the WebUI log pane shows what
    // actually happened (verbose by design, ticket #224).
